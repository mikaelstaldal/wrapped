...
One failure mode is worth recognising, because it looks like a wrapped bug and leaves no trace in the AppArmor logs: if a profile confining the `wrapped` binary execs `systemd-run` with an uppercase mode (`Px`, `Ux`, `Cx`), AppArmor scrubs the environment through the kernel's unsafe-exec path, setting `AT_SECURE`. systemd reads the bus variables with `secure_getenv()`, which returns nothing under `AT_SECURE`, so `systemd-run` reports `Failed to connect to bus: No medium found` no matter what environment wrapped hands it. The fix is in the profile (a lowercase `ix`/`px`/`ux`), not in this code — see the README.
